...
@time for expname in ["iter129_bulkformula"]
    ...
    @time for tt=1:nt
        ...
        Uθ_conv = MeshArray(γ,Float32,50); fill!(Uθ_conv, 0.0)
        κθ_conv = MeshArray(γ,Float32,50); fill!(κθ_conv, 0.0)

        calc_UV_conv3D!(uθ, vθ, Uθ_conv)
        push!(DiffH, sum(Uθ_conv .* PAC_msk) / sum_masked_volume)

        calc_UV_conv3D!(κuθ, κvθ, κθ_conv)

        push!(AdvH, sum(κθ_conv .* PAC_msk) / sum_masked_volume)
...
jldsave(datadir("HeatBudgetTrue_"*region*"_full_2to3_2.jld2"); Heat_Budgets)
plot(Heat_Budgets["iter129_bulkformula"]["DiffH"])


plot(sum_masked_volume .* derivative(Heat_Budget_true2["DiffH"], 2.628e+6))
plot!(sum_masked_volume .* derivative(Heat_Budget_true["AdvH"], 2.628e+6))
