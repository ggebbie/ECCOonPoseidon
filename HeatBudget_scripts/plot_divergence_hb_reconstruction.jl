...
start = 0.0 
GTF_temp = reconstruct(start, fill(GTF, length(tecco[2:end])))
...
for (i, var) in enumerate([θCAR, θCAH, θCDH, θCDR])
    ...
    for key in keys(shortnames )
        start = θz[key][1]
        temp_dict[key] =  reconstruct(start/4, var[key][2:end])
    end
...
sns.move_legend(ax[4], "lower center", bbox_to_anchor=(.5, -.5), ncol=3)
fig.tight_layout()
fig.savefig(plotsdir() * "/OHC_Divergence/Reconstructions/" * "θReconstructAllTerms_" * region * suffix * ".png")
close("all")
...
fig.tight_layout()
fig.savefig(plotsdir() * "/OHC_Divergence/Reconstructions/" * "θReconstruction_" * region * suffix * ".png")
close("all")
...
# ax.legend(handles = patches)
# fig.tight_layout()
# fig.savefig(plotsdir() * "/OHC_Divergence/Reconstructions/" * "θReconstructionApprox_" * region * suffix * "_2km3km.png")
# close("all")
####EXTRA STUFF TO GET LEFTOVER GTH
remain=Float32.(θz["iter129_bulkformula"][2:end].-θz["iter129_bulkformula"][1:end-1])/(Δt₂)
test=cumsum(vcat(θz["iter129_bulkformula"][1], remain .* Δt₂))
Float32(θz["iter129_bulkformula"][224])+(tecco[224]*Float32(Δt₂))
...
println("residual heating: ", Gtf/len_in_sec)
