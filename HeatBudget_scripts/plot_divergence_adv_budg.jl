...
fig, axsm = plt.subplots(4, 3, figsize =(21, 16), sharey = "col", sharex = "col");
...
axsm[1, 1].set_title("Lateral Advection \n" * vθ_sym)
axsm[1, 2].set_title("Vertical Advection \n" *wθ_sym)
axsm[1, 3].set_title("Sum \n" * vθ_sym * L"+" * wθ_sym)
...
[ax.set_xlabel("time") for ax in axsm[4, :]]
axsm[4, 2].legend(); 
sns.move_legend(axsm[4, 2], "lower center",  bbox_to_anchor=(.5, -0.3), ncol=3, frameon=true, borderaxespad=0.)

# fig.tight_layout()
# fig.savefig(plotsdir() * "/OHC_Divergence/Reconstructions/" * "AdvHeadBudget_" * region * "_" * suffix * ".png",
# dpi = 1000)
