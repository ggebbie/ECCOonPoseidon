...
function basin_mask(basin_names::Vector,γ;hemisphere=nothing,southlat=nothing,northlat=nothing,Lsmooth=nothing)
    ...
    if !isnothing(Lsmooth) #this has to be last otherwise it won't smooth all edges!
...
"""
    ...
    overlay a southerly and northerly latitude mask on previous 'mask'
    southlat: specify south latitude boundary of mask (inclusive)
    northlat: specify north latitude boundary of mask (inclusive)
