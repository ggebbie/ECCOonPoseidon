...
"""
function extract_meridional_Ψ_Eul(expname::String,diagpath::Dict, 
    Γ::NamedTuple, γ::gcmgrid, mask::MeshArray)
    fileroot = "trsp_3d_set1"
    filelist = searchdir(diagpath[expname],fileroot) # first filter for state_3d_set1
    datafilelist  = filter(x -> occursin("data",x),filelist) # second filter for "data"
...
println("Transport value: ", result)
"""
function ThroughFlow(Utrsp::MeshArrays.gcmarray{T, 2, Matrix{T}}, 
    Vtrsp::MeshArrays.gcmarray{T, 2, Matrix{T}},IntegralPath) where {T<:Real}
    nz = size(Utrsp, 2)
    trsp=zeros(T, nz)
    ...
    for iz = 1:nz
        for k=1:nW
            (a,i1,i2,w)=tabW[k,:]
            u=Utrsp.f[a, iz][i1,i2]
            trsp[iz]=trsp[iz]+w*u
        end
        for k=1:nS
            (a,i1,i2,w)=tabS[k,:]
            v=Vtrsp.f[a, iz][i1,i2]
            trsp[iz]=trsp[iz]+w*v
        end
    end
    return trsp
