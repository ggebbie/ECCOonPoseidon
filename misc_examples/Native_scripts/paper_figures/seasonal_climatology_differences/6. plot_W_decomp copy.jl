...
vars =  ["iter0_bulkformula","only_wind", "mean_tau_adjusts"]
WEulBol = Dict(); dθdz = Dict(); 
...
for (i, expname) in enumerate(vars)
    ...
    WEulBol[expname] = -Float32.(Ψ_EulBol[z_ref, ϕ_ref[1], :])
end
lw = 3

exp_colors["mean_tau_adjusts"] = "purple"
vars =  ["iter0_bulkformula","only_wind", "mean_tau_adjusts"]
fig, ax = plt.subplots(1, 2, figsize = (10, 7))
for (i, expname) in enumerate(vars)
    Wres = 1e-6 .* WEulBol[expname][1, :][:]
...
vars =  ["only_wind", "mean_tau_adjusts"]
[WEulBol[expt] .-= WEulBol["iter0_bulkformula"] for expt in vars]

vars =  ["only_wind", "mean_tau_adjusts"]
fig, ax = plt.subplots(1, 2, figsize = (17, 5), sharex = true)
for (i, expname) in enumerate(vars)
    Wres = 1e-6 .* WEulBol[expname][1, :][:]
    ax[1].plot(tecco, Wres, label = L"W_{res}' |_{z = 2000}", 
