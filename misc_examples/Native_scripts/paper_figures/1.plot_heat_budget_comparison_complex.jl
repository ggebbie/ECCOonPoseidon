...
ext_advection["Difference"] = ext_advection["iter129_bulkformula"] .- ext_advection["iter0_bulkformula"]
int_advection["Difference"] = int_advection["iter129_bulkformula"] .- int_advection["iter0_bulkformula"]
...
for (i, expname) in enumerate(exps)
    ...
    axs[1].plot(tecco, 100 .* (temps[expname] .- temps[expname][1]), label = plot_labels[expname], c = exp_colors[expname], linewidth = lw)

    axs[2].plot(tecco, integration(0, ext_advection[expname]), label = plot_labels[expname], c = exp_colors[expname], linewidth = lw)
    trends_dict[expname][1] =  trend(integration(0, ext_advection[expname]))

    axs[3].plot(tecco, integration(0, int_advection[expname]), label = plot_labels[expname], c = exp_colors[expname], linewidth = lw)
    trends_dict[expname][2] =  trend(integration(0, int_advection[expname]))

    axs[4].plot(tecco, integration(0, diffusion[expname]), label = plot_labels[expname], c = exp_colors[expname])
    trends_dict[expname][3] =  trend(integration(0, diffusion[expname]))

    axs[5].plot(tecco, integration(0, GTF[expname]), label = plot_labels[expname], c = exp_colors[expname], linewidth = lw)
...
trends_dict["Difference"] = zeros(5)
...
axs[1].legend(loc = "lower left", frameon = false, bbox_to_anchor = (0.1, -0.5), ncols = 1)
# fig.subplots_adjust(wspace = 0.2)
fig
fig.savefig(plotsdir("native/paper_figures/1.HeatBudgetTimeSeries_complex.png"), bbox_inches = "tight", dpi = 1000)
