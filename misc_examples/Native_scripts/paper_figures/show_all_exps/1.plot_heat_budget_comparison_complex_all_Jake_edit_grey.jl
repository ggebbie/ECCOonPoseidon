...
trends_dict = Dict()
for (i, expname) in enumerate(exps)
    if (expname == "iter0_bulkformula") |  (expname == "iter129_bulkformula")
        alpha =  1
    else
        alpha =  0.2
    end
    ...
    println(expname)
    c = exp_colors[expname]

    axs[1].plot(tecco, 100 .* (temps[expname] .- temps[expname][1]), label = plot_labels_list[i], c = c, linewidth = lw, alpha = alpha)



    axs[2].plot(tecco, integration(0, int_advection[expname]), label = plot_labels_list[i], c = c, linewidth = lw, alpha = alpha)
    trends_dict[expname][2] =  trend(integration(0, int_advection[expname]))

    axs[2].plot(tecco, integration(0, diffusion[expname]), label = plot_labels_list[i], c =c, alpha = alpha)
    trends_dict[expname][3] =  trend(integration(0, diffusion[expname]))

    axs[2].plot(tecco, integration(0, GTF[expname]), label = plot_labels_list[i], c = c, linewidth = lw, alpha = alpha)
    trends_dict[expname][4] =  trend(integration(0, GTF[expname]))

    axs[2].plot(tecco, integration(0, ext_advection[expname]), label = plot_labels_list[i], c = c, linewidth = lw, zorder = 10, alpha = alpha)
...
fig
fig.savefig(plotsdir("native/paper_figures/1.HeatBudgetTimeSeries_complex_all_2panels_grey1.png"), bbox_inches = "tight", dpi = 400)
