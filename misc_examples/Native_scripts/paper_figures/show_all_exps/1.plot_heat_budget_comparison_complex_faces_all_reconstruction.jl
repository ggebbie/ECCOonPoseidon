...
for (i, expname) in enumerate(exps)
    ...
    println(expname)
    axs[1].plot(tecco, integrate(0, Topadvection[expname]), label = plot_labels_list[i], c = exp_colors[expname], linewidth = lw)
    trends_dict[expname][1] =  trend(integrate(0, Topadvection[expname]))
    axs[2].plot(tecco, integrate(0, Botadvection[expname]), label = plot_labels_list[i], c = exp_colors[expname], linewidth = lw)
