...
sum((OHC_helper.get_cell_volumes(area .* PAC_msk, cell_depths))) / sum((OHC_helper.get_cell_volumes(area, cell_depths)))
...
vars = ["seasonalclimatology_iter0"]

for expname in vars
    vars = filter_heat_budget(diagpath, expname, γ)
    savename = datadir("native/" * expname * region * "_THETA_budget_ref_" * suffix * ".jld2")
