...
fig, axess = plt.subplots(2, 2, figsize = (14, 10), sharex = true, sharey = true)
...
for (i, expname) in enumerate(["iter129_bulkformula", "iter0_bulkformula"])
    ...
    axes[1].plot(tecco[1:nt], -100 .* integrate(0, wθTop) .* 2.628e+6, label = L"\mathcal{A}_T (w \theta)", alpha = 0.8, linewidth = lw, c = colors[5], zorder = 0)
    axes[1].plot(tecco[1:nt], -100 .* integrate(0, vars["Wbθb_Top"]) .* 2.628e+6, alpha = 1, label = L"\mathcal{A}_T (\overline{w} \overline{\theta})", linewidth = lw, c = colors[1])
    axes[1].plot(tecco[1:nt], -100 .* integrate(0, vars["Wbθp_Top"]) .* 2.628e+6, alpha = 0.7, label = L"\mathcal{A}_T (\overline{w} \theta')", linewidth = lw, c = colors[2])
    axes[1].plot(tecco[1:nt], -100 .* integrate(0, vars["Wpθb_Top"]) .* 2.628e+6, alpha = 0.7, label = L"\mathcal{A}_T (w' \overline{\theta})", linewidth = lw, c = colors[3])
    axes[1].plot(tecco[1:nt], -100 .* integrate(0, vars["Wpθp_Top"]) .* 2.628e+6, alpha = 0.7, label = L"\mathcal{A}_T (w' \theta')", linewidth = lw, c = colors[4])
    axes[1].legend(markerscale = 0.1, ncols = 1, columnspacing = 0.5, frameon = false)

    axes[2].text(0.6, 0.92, alabels[i], transform=axes[2].transAxes, fontweight = "bold")
    axes[2].plot(tecco[1:nt], 100 .* integrate(0, wθBot) .* 2.628e+6, label = L"\mathcal{A}_B (w \theta)", alpha = 0.8, linewidth = lw, c = colors[5], zorder = 0)
    axes[2].plot(tecco[1:nt], 100 .* integrate(0, vars["Wbθb_Bottom"]) .* 2.628e+6, alpha =1 , label = L"\mathcal{A}_B (\overline{w} \overline{\theta})", linewidth = lw,  c = colors[1])
    axes[2].plot(tecco[1:nt], 100 .* integrate(0, vars["Wbθp_Bottom"]) .* 2.628e+6, alpha = 0.7, label = L"\mathcal{A}_B (\overline{w} \theta')", linewidth = lw, c = colors[2])
    axes[2].plot(tecco[1:nt], 100 .* integrate(0, vars["Wpθb_Bottom"]) .* 2.628e+6, alpha = 0.7, label = L"\mathcal{A}_B (w' \overline{\theta})", linewidth = lw, c = colors[3])
    axes[2].plot(tecco[1:nt], 100 .* integrate(0, vars["Wpθp_Bottom"]) .* 2.628e+6, alpha = 0.7, label = L"\mathcal{A}_B (w' \theta')", linewidth = lw, c = colors[4])
...
fname = region * "_THETA_INT_budget_ref_reynolds_iter129iter0" * suffix * ".png"
