...
eff_exps = ["only_init", "only_kappa", "only_wind", "only_sfc", "only_buoyancy", "iter129_bulkformula", "iter0_bulkformula"]

full_exps_trends = Dict()
full_exps_trends = Dict(key => compute_depth_trends(adjust_exps[key]) for key in keys(adjust_exps))
...
eff_exps = ["iter0_bulkformula", "only_init", "only_kappa", "only_sfc", "iter129_bulkformula"]

spice_exps = jldopen(datadir(region * "_temp_trends_sigma2_to_z.jld2"))["trends"]
spice_exps_trends = Dict()
mid_depth_mean(x) = vec(sum(Float32, x[lvls] .* ΔV[lvls], dims = 1) / sum(Float32, ΔV[lvls]))
[spice_exps_trends[key] = 1000 .* mid_depth_mean(spice_exps[key]) for key in eff_exps] #remove the effect of the baseline
...
ax[1].set_ylabel("cK per decade", fontweight = "bold")
[a.set_ylim(-0.75, 0.75) for a in ax]
...
fig
fig.savefig(plotsdir("native/sensitivity_exps/0.North_Pacific_Temp_HeaveSpiceDecomp.png"), bbox_inches = "tight")
fig
