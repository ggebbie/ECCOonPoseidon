...
function lin_regress(x1, x2, y)
    nt = length(y)
    E = ones(Float32, nt, 2) #x1, x2, intercept
    E[:, 1] .= x1 .+ x2
...
# EulBolRegCoef = DataFrame(Dict("Eul" => zeros(3), "Bol" => zeros(3),  "c" => zeros(3), "expname" => Array{String}(undef, 3)))
...
for (i, expname) in enumerate(vars)
    ...
    x1 = -(86400 * 365 * 10) * WEul[expname] / 100
    x2 = -(86400 * 365 * 10) * WBol[expname] / 100
    ...
    b, ϵ = Float32.(lin_regress(x1, x2, y1))
    BolRegCoef.b[i] = b
    EulRegCoef.b[i] = b

    ỹ = (b .* x1) .+ (b .* x2); 
    println("Full Rsq")
    println("R² = ", cov(y1,ỹ ) / (std(y1) * std(ỹ)))
