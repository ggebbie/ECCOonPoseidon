...
τ = τ[2:end-1, :]
predi = -(ΨEulBol["only_wind"][:, 1:end-2, :] .- ΨEulBol["only_wind"][:, 3:end, :])
...
ax2 = fig.add_subplot(7, 1, (3, 7), sharex=ax1)
...
ax2.invert_yaxis()
ax1.plot(ϕ_avg[2:end-1, :], mean(τ, dims = 2), c = "k");
...
issig = ΨCorr .* 1; issig[abs.(issig) .>= 0.7] .= NaN; issig[abs.(issig) .< 0.7] .= 1

X = ϕ_avg[2:end-1]' .* ones(length(z))
Y = ones(length(ϕ_avg[2:end-1]))' .* z

# axs.scatter(X, Y, c = issig, cmap = "binary", s=2, alpha=0.2)  
axs.invert_yaxis()
fig.colorbar(CM)
fig

##############
predi = -(ΨBol["only_wind"][:, 1:end-2, :] .- ΨBol["only_wind"][:, 3:end, :])
