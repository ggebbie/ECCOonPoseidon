...
f3_ax2 = fig.add_subplot(obj(slice(0, 2) ,slice(1, 3)), projection=proj0)
...
for ff = 1:5
    cf = axs[1].pcolormesh(λ[ff], ϕ[ff],  β[ff]  .* 100 * 100, transform=projPC, 
...
cbar = axs[2].pcolormesh(LONS, LATS,  (β .* 100 * 100), transform=projPC, 
    cmap = cmos.balance, vmin = -bounds, vmax = bounds)   
axs[2].set_title("OPT-15")
for ax in axs
    ax.coastlines(resolution="110m")
    ax.set_extent((0, 360, -70, 60),crs=projPC)
