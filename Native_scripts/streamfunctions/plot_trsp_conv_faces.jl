...
Ψmaxidx = Base.findmin(abs.(ϕ_avg .- 2))[2]
...
for (i, var) in enumerate(["iter0_bulkformula", "iter129_bulkformula"])
    println("-----")
    println(var)
    fname = datadir("Ψ_EulBol_timeseries_PAC_" * var *".jld2")
    Ψ = load(fname)["Ψ_exp_timeseries"]
    Ψ_max = -1e-6 .* Ψ[45, Ψmaxidx, :]
    Ψ_max = Ψ_max .-mean(Ψ_max)
    println(Ψ_max)
    Vin = 1e-6 .* (Ψ[lvls[end] + 1, NPAC_boundidx, :] .- Ψ[lvls[1], NPAC_boundidx, :])
    Wtop = 1e-6 .* Ψ[lvls[1:2] .- 1, NPAC_boundidx, :]
    Wtop = mean(Wtop, dims = 1)[1, :]

    Wbot = -1e-6 .* Ψ[lvls[end-1:end] .+ 1, NPAC_boundidx, :]
    Wbot = mean(Wbot, dims = 1)[1, :]
    axes = ax[i, :]
    ...
    axes[4].plot(tecco, Ψ_max, c = colors[i], alpha = 0.2); 
    ...
    axes[4].plot(tecco, extract_interannual(-Ψ_max), label = "Ψ_max" * "\n trend: "  * round_trend(Wbot), c = colors[i], alpha = 1, linewidth = 2)
