...
function get_densities(expname)
    ...
    N²z_mean = zeros(Float32, nt, 50)
    N² = MeshArray(γ,Float32,nz)
    for tt in 1:2
        ...
        Cθz = gsw_ct_from_pt.(Sz, θz)
        for ijk in eachindex(P)
            N².f[ijk] .= gsw_nsquared.(Cθz.f[ijk], Sz.f[ijk], P.f[ijk], 45.0)
        end 
        N²z_mean[tt, :] .= zonal_average(N², cell_volumes)
    end
    return N²z_mean
end

σz_mean = Dict()
get_densities("iter0_bulkformula")
gsw_ct
