...
eff_exps = ["mean_tau_yesadjust_redo_bf", "mean_tau_noadjust_redo_bf", "DIFF"]
zs = z_dict["mean_tau_noadjust_redo_bf"]

sns.set_style("darkgrid", Dict("axes.facecolor" => ".95", "grid.color" => "0.5"))


fig, ax = plt.subplots(3, 3, figsize = (15,12), sharey = "row", sharex = true)
fig.tight_layout()
include(srcdir("plot_and_dir_config.jl"))
...
for (i, expt) in enumerate(eff_exps)
    ...
    for (j, var) in enumerate([spice .+ heave, heave, spice])
...
[a.set_ylabel("Depth [m]", fontweight = "bold") for a in ax[:, 1]]
[a.set_xlabel("time", fontweight = "bold") for a in ax[end, :]]
...
ax[1, 3].annotate(plot_labels_list[2], (1.15, 0.2), fontsize =  19, rotation = 270, 
xycoords="axes fraction", ha="center", fontweight = "bold", color = exp_colors["only_wind"])
ax[2, 3].annotate(plot_labels_list[1], (1.15, 0.2), fontsize =  19, rotation = 270, 
xycoords="axes fraction", ha="center", fontweight = "bold", color = exp_colors["iter0_bulkformula"])
ax[3, 3].annotate("Model\nDifference", (1.15, 0.3), fontsize =  19, rotation = 270, 
...
for (i, color) in enumerate(colors)
    ...
    pos2 = ax[i, end].get_position()
    
    # Create a rectangle that spans the entire row
    rect = patches.Rectangle(
        (pos1.x0 - 0.08, pos1.y0 - 0.005),  # Bottom-left corner
        (pos2.x1 - pos1.x0) + 0.17,  # Width (span from first to last subplot)
        pos1.y1 - pos1.y0 + 0.010,  # Height (of the subplot row)
        linewidth=2.5,
        edgecolor=color,
        facecolor="none",
    ...
    rect = patches.Rectangle(
        (pos1.x0, pos1.y1 + 0.015),  # Bottom-left corner
        (pos1.x1 - pos1.x0) ,  # Width (span from first to last subplot)
         0.06,  # Height (of the subplot row)
        linewidth=2.5,
        edgecolor="none",
        facecolor="lightgrey",
