...
[spice_component[key] = spice_exps[key] for key in eff_exps] #remove the effect of the baseline
...
x = heave_component["DIFF"][lvls, end] .- heave_component["DIFF"][lvls, 1]
...
axs[1, 2].set_title("Heave Signal")
axs[1, 3].set_title("Water Mass Signal")
for (i, expt) in enumerate(["iter129_bulkformula", "iter0_bulkformula", "DIFF"])
    ax = axs[i, :]
    spice = spice_component[expt]; 
    heave = heave_component[expt];
    vmax = 1.4
    levels = -vmax:0.2:vmax
    plot_contour(ax, c) = ax.contourf(tecco, z ./ 1000, 100 .* (c .- mean(c, dims = 2)), 
    ...
    ax[1].text(-0.6, 0.5, labels[i], transform = ax[1].transAxes, 
...
colorbar = fig.colorbar(cmaps, ax = axs[:], label = "cK", 
