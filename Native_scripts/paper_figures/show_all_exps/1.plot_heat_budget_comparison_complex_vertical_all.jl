...
for (i, expname) in enumerate(exps)
    ...
    println(expname)
    axs[1].plot(tecco, integrate(0, Vadvection[expname] .+ Hadvection[expname]), label = plot_labels[expname], c = exp_colors[expname], linewidth = lw)
    trends_dict[expname][1] =  trend(integrate(0, Vadvection[expname] .+ Hadvection[expname]))
    axs[2].plot(tecco, integrate(0, Vadvection[expname]), label = plot_labels[expname], c = exp_colors[expname], linewidth = lw)
    trends_dict[expname][2] =  trend(integrate(0, Vadvection[expname]))

    axs[3].plot(tecco, integrate(0, Hadvection[expname]), label = plot_labels[expname], c = exp_colors[expname])
...
trends_dict["Difference"] = zeros(3)
...
trends_df
[a.grid() for a in axs]
axs[2].legend(loc = "lower center", frameon = false, bbox_to_anchor = (0.0, -0.2), ncols = 5)
fig.tight_layout()
fig.subplots_adjust(wspace = 0.1)
fig
fig.savefig(plotsdir("native/paper_figures/2.HeatBudgetTimeSeries_external_decomp_all.png"), bbox_inches = "tight", dpi = 400)
