...
eff_exps = ["iter129_bulkformula", "only_wind", "only_init", "only_kappa", "iter0_bulkformula"]
...
fig, ax = plt.subplots(5, 2, figsize = (15,25), sharey = "row", sharex = false)
...
for (i, expt) in enumerate(eff_exps)
    ...
    vmax = 2.8; levels = -vmax:0.4:vmax

    plot_contourf(ax, c) = ax.contourf(tecco,  z[20:46], 100 .* c[20:46, :], vmin = -vmax, vmax = vmax, levels = levels, cmap = cm.balance, extend = "both")
    plot_contour(ax, c) = ax.contour(tecco,  z[20:46], 100 .* c[20:46, :], levels = levels,colors="k")  # Negative contours default to dashed.
