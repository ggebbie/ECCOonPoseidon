...
MeshArrays, MITgcmTools, JLD2, DrWatson, Statistics, JLD2, Printf, 
DataFrames, LaTeXStrings, GibbsSeaWater
...
function area_average_by_σ(ds::MeshArray, ΔA::MeshArray)
    ...
    for k=1:nz 
        ΔA_mask = 1 .* ΔA[:, k] #this should shrink with changing isopycnal surfaces
        for ff=1:5
            drypts = isnan.(ds_mask .* ds[ff, k])
