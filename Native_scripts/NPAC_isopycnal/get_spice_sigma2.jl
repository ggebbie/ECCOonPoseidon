...
for expname in ["only_init", "only_kappa", "only_buoyancy", "only_wind", "iter0_bulkformula", "iter129_bulkformula"]
...
for (i, expname) in enumerate(["only_init", "only_kappa", "only_buoyancy", "only_wind", "iter0_bulkformula", "iter129_bulkformula"])
    z_levs = 1500 .< z_dict[expname] .< 3500
    θ = 100 .* θ_dict[expname][z_levs, :]
    axs[i].contourf(tecco, z_dict[expname][z_levs], θ .- mean(θ, dims = 2), 
    cmap = cmo.balance, vmin = -1, vmax = 1, levels = 10, extend = "both")
