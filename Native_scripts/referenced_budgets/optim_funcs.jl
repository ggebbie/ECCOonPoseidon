...
loss_function(y1, y2, t, x) = sum_squares(y2, t,  x[2:end]) .+ sum_squares(y1, t,  x[[1, 3, 4]])
...
function error_bars(y1, ỹ1, y2, ỹ2, t, t_est)
    ...
    x_ens = zeros(4, nrealizations)
    for i in 1:nrealizations
        x0 = [0.1, 0.1, 0.01, 1.5] #initial guess
        y1_samp = y1 .+ rand(n1, nt)
        y2_samp = y2 .+ rand(n2, nt)

        g(x) = loss_function(y1_samp, y2_samp, t, x, p)
        sol = optimize(g1, x0, NelderMead())
        a1, a2, b, c = Optim.minimizer(sol)

        x_ens[:, i] .= [a1, a2, b, c]
        ỹ1_sims[:, i] .= obj_func(t_est, [a1, b, c])
        ỹ2_sims[:, i] .= obj_func(t_est, [a2, b, c])
    end
...
function plot_mean_and_error!(ax, t, y_ens)
    ens_avg, ens_σ = mean_std(y_ens)
    ax.plot(t, ens_avg, color = "k")
    ax.fill_between(t, ens_avg .- (2 .*ens_σ), ens_avg .+ (2 .*ens_σ), alpha=0.3, color="k")
end
