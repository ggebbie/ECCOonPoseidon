...
outputdir = "/vast/eccodrive/files/Version4/Release4/other/input_forcing_unadj_climatological_test/"
...
varnames = ["eccov4r4_unadj_ustr_"]
frootsample = inputdir*varnames[1]*midname
...
t14day = range(t14day_start,step=Δt14day,stop=t6hr[end] +14)
...
overtones = 5
Ecycle,Fcycle = seasonal_matrices(fcycle,t14day,overtones)

for vname ∈ varnames
    filein = inputdir*vname*midname
    fileout = outputdir*vname*midname
    println("Processing: ", vname*midname)
...
ax.plot(t, seasonal_saved, linewidth = 2, 
...
#prev error was -9e-5
#error using 14 day end date is 