...
inputdir = "/vast/eccodrive/files/Version4/Release4/other/input_forcing_unadjusted/"
outputdir = "/vast/eccodrive/files/Version4/Release4/other/input_forcing_unadj_climatological/"
...
Ecycle,Fcycle = seasonal_matrices(fcycle,t14day,4)

# _,w = seasonal_matrices(fcycle,t6hr,-1); w = w[1]
...
for vname ∈ varnames
    ...
    # iNX, iNT = size(flux_14day_seasonal)

    # seasonal_mean =  MeshArray(γ,Float32); fill!(seasonal_mean,0.0)
   
    # for i in 1:iNT
    #     seasonal_mean.f[:] .+= (flux_14day_seasonal.f[:, i] ./ iNT) 
    # end

    # println(sum(true_mean))
    # println(sum(seasonal_mean))

    # cons_offset!(flux_14day_seasonal, -seasonal_mean) #remove the iteration 129 mean 
    # cons_offset!(flux_14day_seasonal, true_mean) #remove the iteration 129 mean 

    # check for NaN's in output
    (sum(nancount(flux_14day_seasonal)) > 0) && (error("NaNs in the filtered output"));

    ECCOtour.matrixsaveinterpolation(E14to6,flux_14day_seasonal,filein,fileout,years,γ)
end
...
ax.plot(t, actual, label = "Original USTRESS [Δt=6 hours]"); ax.set_xlabel("years")
ax.plot(t, seasonal_saved, linewidth = 4, 
label = "Seasonal Cycle of USTRESS [Δt=6 hours]");
