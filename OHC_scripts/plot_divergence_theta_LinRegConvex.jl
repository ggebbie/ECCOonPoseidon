...
df = DataFrame(θz);
A = zeros(312, 2)
A .= Matrix{Float32}(df[!, ["noinitadjust",  "nosfcadjust"]]) .- 
...
path = "/OHC_Divergence/Reconstructions/" * "θPredictionsLIN_" * region * "_" * suffix * ".png"
fig.tight_layout(); fig.savefig(plotsdir() * path,
dpi = 1000)
