...
function extract_meridionalΨEK(expname,diagpath,fileroot, Γ, γ, Hinv, finv, mask)
    ...
    datafilelistJan = datafilelist[1:12:end]
    datafilelistJul = datafilelist[7:12:end]
    LC=LatitudeCircles(-89.0:89.0,Γ)
    nz=size(Γ.hFacC,2); nl=length(LC); nt = length(datafilelistJan)
    Ψs = zeros(nt, nl, nz)
    ...
    Threads.@threads for tt=1:nt
        fnameJan = datafilelistJan[tt]
        fnameJul = datafilelistJul[tt]
        EXFJan = γ.read(diagpath[expname]*fnameJan,MeshArray(γ,Float32,15))
        EXFJul = γ.read(diagpath[expname]*fnameJul,MeshArray(γ,Float32,15))
        EXF = EXFJan .- EXFJul
        EXF = EXF .* mask; τx = EXF[:, 14]; τy = EXF[:, 15]
        U = MeshArray(γ,Float32,50); V = MeshArray(γ,Float32,50);
        getUeVe!(U, V, τx, τy, finv, Hinv, hinv);
        U = U .* hFacC; V = V .* hFacC 
...
function plot_Ψ_mean(X, Y, Ψ_zonal, Ψ_labels, clims, Ψ_sym)
    ...
    plot_title =  "Mean Seasonal (Jan - July) "* Ψ_sym  * " "* region,
...
savefig(p,plotsdir() * "/OHC_Divergence/MassTransport/" * "ΨEKmeanszn_"* 
region * ".png")
...
savefig(p,plotsdir() * "/OHC_Divergence/MassTransport/" * "ΨEKmeanszn_0anom_"* 
region * ".png")
...
p = plot_Ψ_mean(X, Y, Ψ_exp_mean_anom, Ψ̄_anom_labels, clims, Ψ_sym)
savefig(p,plotsdir() * "/OHC_Divergence/MassTransport/" * "ΨEKmeanszn_129anom_"* 
region * ".png")
