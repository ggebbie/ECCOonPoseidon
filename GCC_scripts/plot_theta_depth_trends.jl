...
labels = ["ECCO", "ECCO Forcing", "ECCO without Forcing Adjustment", "CTRL"]
...
@time for (i, expname) in enumerate(["iter129_bulkformula", "noinitadjust", "nosfcadjust"])#enumerate(keys(shortnames))
    ...
    if i != 2
        ...
        ax.plot(1e2.*β, -z.*1e-3, color = colors[i], label = labels[i])
...
fig
fig.savefig(plotsdir() * "/AGU_Plots/DepthTrendsPACcrop_" * region * "nosfc" * ".pdf",bbox_inches = "tight")
