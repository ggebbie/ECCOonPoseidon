...
for (i, expname) in enumerate(keys(shortnames))
    ...
    E_trsp = Vtrsp
    F_trsp = (E_trsp'*E_trsp)\E_trsp' # least squares estimator
    Vθsouth_true = derivative(Heat_Budget_true["AdvH"], 2.628e+6) .* NPAC_Volume
    θ_est = F_trsp*Vθsouth_true

    meridional_advection_approx = (θ_est .* Vtrsp) ./ NPAC_Volume
    meridional_advection_approx_mean = cumsum(cat(0.0, meridional_advection_approx .* 2.628e+6, dims = 1))
    Vθp = (Vθsouth_true .- (θ_est .* Vtrsp))./ NPAC_Volume 
    ...
    if i == 1 
        ...
        linestyle = ":", label = L" \int  \frac{\partial}{\partial y} (V \theta') dt" ); 
        axs[1].plot(tecco, meridional_advection_approx_mean[1:end-1], color = cmaps[2], 
        linestyle = "--", label = L" \int  \frac{\partial}{\partial y} (V \bar{\theta}) dt" ); 
        axs[1].plot(tecco, meridional_advection, color = cmaps[1], label = "SUM"); 
        axs[1].set_ylabel("[" * L"^\circ " * "C]")
    end
    axs[2].plot(tecco, 1e-6 .* Vtrsp, color = colors[i], label =labels[i]); 
    Vtrsp_mean.+=(1e-6 .* Vtrsp)./4
    println(expname)
