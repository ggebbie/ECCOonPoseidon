...
function filter_heat_budget(diagpath::Dict{String, String}, expname::String, γ::gcmgrid)
    ...
    @time for tt = 1:nt
        ...
        sθ_0 = extract_sθ("mean_tau_noadjust_redo_bf",diagpath, γ, fnameS, fnameθ, inv_H)
        ...
        _, _, w0, _, _, Wb0 = extract_eulerian_and_bolus_velocities(diagpath, "mean_tau_noadjust_redo_bf", datafilelist_uvw[tt], 
                                                                      γ, Γ, mskC, mskW, mskS)
...
for expt in ["mean_tau_noadjust_redo_bf", "mean_tau_yesadjust_redo_bf"]
    println(expt)
    vars = filter_heat_budget(diagpath, expt, γ)
    savename = datadir("native/" * expt * region * "_THETA_budget_Wθ_eul_bol_ΔDecomp_climatological" * suffix * ".jld2")
