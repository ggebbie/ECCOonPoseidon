...
for (i, var) in enumerate(vars)
    ...
    # contourf!(p, tecco, -z[iz][:], 100 .* var, 
    # color = :balance, levels = ,   
    # yflip = true, ylims = (), lw=0.5, clabels=true, cbar=false)
    # push!(plts, p)
end
axs[1].set_ylabel("Depth [m]")
axs[1].set_ylim(500, 3200)
axs[1].invert_yaxis()
# fig.colorbar(cfs[1], ax=axs, orientation = "horizontal", 
# fraction=0.05, pad = 0.15, extend = "both", label = L"Sv")
